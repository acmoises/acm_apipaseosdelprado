            ...
            <table id="payment-table" class="min-w-full bg-white border border-gray-200 rounded-md shadow-md">
                <thead>
                    <tr class="bg-gray-100 text-gray-700">
                        ...
                        <th class="px-4 py-2 text-left whitespace-nowrap">Recibo de Pago</th> <!-- Nueva columna para acciones -->
                        <th class="px-4 py-2 text-left whitespace-nowrap">Cancelar</th>
                    </tr>
                </thead>
                <tbody>
        ...
        $(document).ready(function() {
            ...
            function loadPaymentsTable(payments) {
                ...
                if (payments.length === 0) {
                    ...
                    payments.forEach(function(payment) {
                        ...
                        let cancelForm = `
                            <form action="/dashboard/payment/cancelled" method="POST" onsubmit="return confirm('¿Estás seguro de que quieres cancelar este pago?');">
                                @csrf
                                <input type="hidden" name="payment_id" value="${payment.id}">
                                <input type="hidden" name="user_id" value="{{ auth()->id() }}">
                                <button type="submit" class="px-4 py-2 bg-red-500 text-white rounded-md hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500" ${buttonClass}" ${isDisabled}>
                                    Cancelar
                                </button>
                            </form>
                        `;
            ...
            $('#resident').on('change', function() {
                ...
                if (residentId) {
                    ...
                    $.ajax({
                        url: "{{ route('payments.byResident') }}", // Ruta para obtener los pagos por residente
                        type: 'GET',
